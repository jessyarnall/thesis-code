...
df = pd.read_csv("/home/jyarnal1/lateral_all.csv", header=0, sep=',')
df = df.drop('view', axis = 1)

df = df.replace(2, 0) #uncertain
df = df.replace(1, 0) #negative
df = df.replace(3, 1) #positive
...
prediction_layer = tf.keras.layers.Dense(14, activation = 'relu')
...
with open("vgg16_lateral_all_epoch10_uncertain0_relu_accuracy.txt", "w") as output:
    output.write(str(accuracyt))

history = history.history

with open("vgg16_lateral_all_epoch10_uncertain0_relu_history.txt", "w") as output:
