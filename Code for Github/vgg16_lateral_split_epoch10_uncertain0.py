...
train_df = pd.read_csv("/home/jyarnal1/lateral_train.csv", header=0, sep=',')
train_df = train_df.drop('split', axis = 1)

test_df = pd.read_csv("/home/jyarnal1/lateral_test.csv", header=0, sep=',')
test_df = test_df.drop('split', axis = 1)

valid_df = pd.read_csv("/home/jyarnal1/lateral_val.csv", header=0, sep=',')
valid_df = valid_df.drop('split', axis = 1)

train_df = train_df .replace(2, 0) #uncertain
train_df = train_df .replace(1, 0) #negative
train_df = train_df .replace(3, 1) #positive

test_df = test_df .replace(2, 0) #uncertain
test_df = test_df .replace(1, 0) #negative
test_df = test_df .replace(3, 1) #positive

valid_df  = valid_df .replace(2, 0) #uncertain
valid_df  = valid_df .replace(1, 0) #negative
valid_df  = valid_df .replace(3, 1) #positive
...
prediction_layer = tf.keras.layers.Dense(14)
...
with open("vgg16_lateral_split_epoch10_uncertain0_accuracy.txt", "w") as output:
    output.write(str(accuracyt))

history = history.history

with open("vgg16_lateral_split_epoch10_uncertain0_history.txt", "w") as output:
